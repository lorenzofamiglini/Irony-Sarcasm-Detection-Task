...
from tensorflow.keras.callbacks import ModelCheckpoint, EarlyStopping, CSVLogger, History
...
def finetuning_callbacks(checkpoint_path, patience, verbose):
    ...
    """
    cb_verbose = (verbose >= 1)
    checkpointer = ModelCheckpoint(monitor='val_loss', filepath=checkpoint_path,
                                   save_best_only=True, verbose=cb_verbose, save_weights_only=True)
    earlystop = EarlyStopping(monitor='val_loss', patience=patience,
                              verbose=cb_verbose)
    history = History()
    return [checkpointer, earlystop, history]
...
def change_trainable(layer, trainable, verbose=True):
...
def sampling_generator(X_in, y_in, batch_size, epoch_size=1000, 
                       upsample=False, seed=42):    #epoch_size default 25000
...
def finetune(model, texts, labels, nb_classes, batch_size, method,
             metric='acc', epoch_size=1000, nb_epochs=20,
    ...
    if method == 'chain-thaw':
        result,hist = chain_thaw(model, nb_classes=nb_classes,
...
def chain_thaw(model, nb_classes, train, val, test, batch_size,
    ...
    train_gen = sampling_generator(X_train, y_train, batch_size,
                                   upsample=False, seed=seed, epoch_size = epoch_size)
    callbacks = finetuning_callbacks(checkpoint_weight_path, patience, verbose)

    # Train using chain-thaw
    model2, history_model = train_by_chain_thaw(model=model, train_gen=train_gen,
    ...
    if evaluate == 'acc':
        return evaluate_using_acc(model2, X_test, y_test, batch_size=batch_size)
    elif evaluate == 'weighted_f1':
        return evaluate_using_weighted_f1(model2, X_test, y_test, X_val, y_val,
                                          batch_size=batch_size)
    
    return model2, history_model
...
def train_by_chain_thaw(model, train_gen, val_data, loss, callbacks, epoch_size,
    ...
    # at the end
    for layer in ['softmax', 'embedding', 'bi_lstm_0', 'bi_lstm_1', 'attlayer', None]:
        ...
        for _layer in ['softmax', 'embedding', 'bi_lstm_0', 'bi_lstm_1', 'attlayer', None]:
            if _layer is not None:
                trainable = _layer == layer or layer is None
                change_trainable(model.get_layer(_layer), trainable=trainable, verbose=True)

        # Verify we froze the right layers
        #for _layer in model.layers:
        #    if _layer is not None and len(_layer.trainable_weights):
        #        assert _layer.trainable == (_layer == model.get_layer(layer)) or layer is None
        ...
        history = model.fit(train_gen, steps_per_epoch=steps,
        ...
        if layer is not None:
            K.clear_session()
            model = deepmoji_transfer(2,50, PRETRAINED_PATH)
            model.load_weights(checkpoint_weight_path, by_name=False)
        else:
            model = deepmoji_transfer(2,50, PRETRAINED_PATH)
            model.load_weights(checkpoint_weight_path, by_name=False)
