...
def _call_runtagger(data, run_tagger_cmd=RUN_TAGGER_CMD):
    ...
    with open("../Code/ark_tweet_pos/temp.txt", "wb") as f:
        f.write(message)
    # build a list of args
    args = shlex.split(run_tagger_cmd) 
    args.append('--output-format')
    args.append('conll')
    args.append('../Code/ark_tweet_pos/temp.txt')
    ...
    pos_result = result[0].decode('utf-8')  # get first line, remove final double carriage return
    pos_result = pos_result.split('\n\n') 
    
    pos_results = [pr.split('\n\r') for pr in pos_result]  # split parts of message by each carriage return
    return pos_results


def runtagger_parse(data, run_tagger_cmd=RUN_TAGGER_CMD):
    """Call runTagger.sh on a list of tweets, parse the result, return lists of tuples of (term, type, confidence)"""
    pos_raw_results = _call_runtagger(data, run_tagger_cmd)[0]
    pos_result = []
    for pos_raw_result in pos_raw_results:
        pos_info = [x[1] for x in re.findall('(\t)(\W|[A-Z])', pos_raw_result)]
        pos_result.append(pos_info)
